# 🏥 APPUNTAMENTI MERCATOPOLI - CON CANCELLAZIONE UTENTE
...
class DatabaseManager:
    ...
    def delete_appointment(self, email: str, data: str, ora: str):
        """Elimina appuntamento - NUOVA FUNZIONALITÀ"""
        appointments = self.file_db.load_appointments()
...
def show_miei_appuntamenti():
    """Mostra appuntamenti utente CON CANCELLAZIONE"""
    ...
    if appointments:
        ...
        if futuri:
            ...
            with st.form("cancella_form"):
                st.warning("⚠️ **Attenzione:** La cancellazione è irreversibile!")
                ...
                if app_options:
                    ...
                    st.info(f"📋 **Dettagli appuntamento:**")
                    ...
                    # Conferma password per sicurezza
                    password_confirm = st.text_input("🔑 Inserisci la tua password per confermare:", type="password")
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.form_submit_button("🗑️ Cancella Appuntamento", type="primary"):
                            if password_confirm:
                                # Verifica password
                                user = db.verify_login(st.session_state.user_email, password_confirm)
                                if user:
                                ...
                                st.error("❌ Inserisci la password per confermare!")
